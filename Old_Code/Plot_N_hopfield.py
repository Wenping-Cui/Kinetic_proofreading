...
def data_ana(N,gamma,bphi):
    Num = 80; # Number of scatter points
...
bphi=0.01
data1 = data_ana(4,bphi,gamma);
data2 = data_ana(8,bphi,gamma);
data3 = data_ana(12,bphi,gamma);
...
data2 = data_ana(8,bphi,gamma);
data3 = data_ana(12,bphi,gamma);
data1 =data1[np.where( data1[:,0] > 0.0 )]
data2 =data2[np.where( data2[:,0] > 0.0 )]
data3 =data3[np.where( data3[:,0] > 0.0 )]
data1 =data1[np.where( data1[:,1] > 10**3 )]
data2 =data2[np.where( data2[:,1] > 10**3 )]
data3 =data3[np.where( data3[:,1] > 10**3 )]
...
ax1.plot(data1[0], data1[2],color=tableau20[2],linewidth=3.0, label=' $N = 4$')
ax1.plot(data2[0], data2[2],color=tableau20[4],linewidth=3.0,label=' $N = 8$')
ax1.plot(data3[0], data3[2],color=tableau20[8],linewidth=3.0, label=' $N = 12$')
ax1.axvline(10**2,ymax=0.9, color='k', linestyle='--',linewidth=3.0)
#ax1.scatter(data4[0], data4[2],color = 'm', marker = '^', label=' $b = 0.001\\phi$')

ax1.set_xscale('log')
#ax1.scatter(speed_boundary, eta_boundary, s=20, c='r')
ax1.set_yscale('log')
ax1.set_ylim([10**(-12),10**1])
ax1.set_xlim([10**(-1),10**7])
...
ax2.axvline(10**2,ymax=0.9, color='k', linestyle='--',linewidth=3.0)
ax2.plot(data1[0], data1[1],color=tableau20[2],linewidth=3.0, label=' $b = 0.1\\phi$')
ax2.plot(data2[0], data2[1],color=tableau20[4],linewidth=3.0,label=' $b = 0.05\\phi$')
ax2.plot(data3[0], data3[1],color=tableau20[8],linewidth=3.0, label=' $b = 0.01\\phi$')
ax2.set_xlim([10**(-1),10**7])
ax2.set_xscale('log')
ax2.set_yscale('log')
...
ax4 = ax2.twinx()
ax4.set_xlim([10**(-1),10**7])
ax4.plot(data1[0], data1[3], 'r--',color=tableau20[2],linewidth=3.0, label=' $b = 0.1\\phi$')
ax4.plot(data2[0], data2[3], 'r--',color=tableau20[4],linewidth=3.0, label=' $b = 0.05\\phi$')
ax4.plot(data3[0], data3[3], 'r--',color=tableau20[8],linewidth=3.0, label=' $b = 0.01\\phi$')
ax4.set_ylim([10**(-3),10**5])
ax4.set_yscale('log')
ax4.set_ylabel('Output', fontsize=25)





ax3.plot(data1[1], data1[2],color=tableau20[2],linewidth=3.0, label=' $b = 0.1\\phi$')
ax3.plot(data2[1], data2[2],color=tableau20[4], linewidth=3.0,label=' $b = 0.05\\phi$')
ax3.plot(data3[1], data3[2],color=tableau20[8],linewidth=3.0, label=' $b = 0.01\\phi$')
...
f.set_size_inches(35, 11)
f.savefig('figure_N_hopfield.png', bbox_inches='tight')
